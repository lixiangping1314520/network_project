    ...
    <el-container>
      ...
      <el-container>
        <el-aside width="200px">        
          <el-tree class="filter-tree" :data="getHeaderDate" :props="defaultProps" default-expand-all :filter-node-method="filterNode" @node-click="handleNodeClick">
            ref="tree2">
          </el-tree>         
        </el-aside>
        <el-container>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapMutations(['setData_prom', 'updateData_prom', 'deleteDate_prom', 'setpname_prom']),
      ...
      update () {
        ...
        this.$http.post('http://localhost:2860/api/ProManage/UpdateProject',
          parm,
          heads
        ).then((response) => {
          if (response === 'success') {
      ...
      deletepro () {
        ...
        this.$http.post('http://localhost:2860/api/ProManage/DeleteProject',
          parm
        ).then((response) => {
          if (response === 'success') {
      ...
      load () {
        this.$http.post('http://localhost:2860/api/ProManage/GetALlProject'
        ).then((response) => {
          this.acSetData_prom(response)
          this.form.id = parseInt(response[0]['id'])
          this.form.name = response[0]['name']
          this.form.createTime = response[0]['createTime']
          this.form.recentlyTime = response[0]['recentlyTime']
          this.form.remarks = response[0]['remarks']
        }).catch((error) => {
          this.$notify.info({
            title: '提示',
            message: '没有相关工程'
    ...
    created: function () {
      if (this.prom.prom_data_list.length === 0) {
        this.load()
      } else {
        console.log('这是create里面的函数 else 函数')
        // this.data_list = this.prom.prom_data_list
        // this.headerData = this.getHeaderDate
      }
    }
  }
</script>
